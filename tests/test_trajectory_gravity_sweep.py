...
class TestTrajectoryVaryingGravity(unittest.TestCase):

    def test_trajectory_for_varying_gravity(self):
        """Test and save particle trajectories for varying gravity values."""
        ...
        # Range of gravity values (from Moon to Earth gravity)
        gravities = [1.62, 3.71, 9.8]  # Lunar gravity, Mars gravity, Earth gravity
        ...
        # Loop through each gravity value, calculate the trajectory, and plot it
        for i, gravity in enumerate(gravities):
            ...
            # Plot the trajectory with a label indicating the gravity value
            plt.plot(y_positions, x_positions, color=colors[i], label=f'g = {gravity:.2f} m/s^2')

        # Add labels, title, and legend to the plot
        plt.title('Particle Trajectories for Varying Gravity Values')
        ...
        output_file = os.path.join(output_dir, 'particle_trajectories_varying_gravity.png')
